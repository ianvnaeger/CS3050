...
int FightClub( struct stats user , struct stats opponent )
{
   // srand(time(NULL));
    ...
    if( oppth > userth )
    {
        printf("\n\n%s stands victorious. %s has been defeated. Finding Weaker Opponent...\n\n", opponent.name, user.name);
        return 0;
    }
   
    if( fightRound == totalRounds )
    {
        printf("\n\nCONGRATULATIONS!!! %s has defeated all those in their way!\n\n\n\tYOU WIN!!!\n\n\n", user.name);
      return 2;
    }
...
{	int level;
	if(!is){
		...
		*defense_level = rand() % 5 + 10;//range is 10 to 15
		level=(*health + *attack_level + *defense_level) / 3;		
	}
    return level;
}
...
Node* Build_List(Node* root, char** Name_Array)//Takes the random stats and name and creates a tree
{
    ...
    {
        name = *(Name_Array + i);
        if(!strcmp("SinorDong", name) || !strcmp("MisterJon", name) || !strcmp("MadamMei", name)) level = Stat_Generator(&health, &attack_level, &defense_level, TRUE);
...
struct node* list_BST(Node **head, int len){//turn sorted linked list to balanced bst helper function
	...
	(*head)->person.health=(*head)->health;
	(*head)->person.attack=(*head)->attack_level;	
	(*head)->person.defense=(*head)->defense_level;		
	
	tree=createNode((*head)->person); tree->left=left;//allocate for left node
	*head=(*head)->next;
